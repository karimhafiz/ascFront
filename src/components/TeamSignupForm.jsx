...
export default function TeamSignupForm({ eventId, managerId, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_DEV_URI}teams/event/${eventId}/unpaid?email=${encodeURIComponent(managerEmail)}`)
      .then((r) => r.json())
          ...
          <div className="mb-5">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-amber-700 mb-3">
                ⚠️ You have {unpaidTeams.length} incomplete registration{unpaidTeams.length > 1 ? "s" : ""} for this tournament:
              </p>
              <div className="space-y-2">
                {unpaidTeams.map((team) => (
                  <div key={team._id} className="flex items-center justify-between bg-white/70 rounded-lg px-3 py-2 border border-amber-100">
                    <div>
                      <p className="font-medium text-purple-800 text-sm">{team.name}</p>
                      <p className="text-xs text-purple-500">{team.members.length} player{team.members.length !== 1 ? "s" : ""}</p>
                    </div>
                    <button
            ...
            <p className="text-xs text-amber-600 mt-3">Or fill in the form below to register a new team.</p>
            </div>
          </div>
        )}
